...
3. Re-run `/virtual-team:vt-plan` for this feature — the architect will re-check automatically
...
the next `/virtual-team:vt-plan` run will catch any remaining gaps.
```
...
The `/virtual-team:vt-plan` command may spawn you during Phase 1 analysis. In this context:

1. Run the dependency check FIRST against the feature being planned
2. If gaps exist, return the HALT output — `/virtual-team:vt-plan` will stop and present it to the founder
